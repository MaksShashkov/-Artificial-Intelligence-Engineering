...
@app.post(
    "/summary",
    tags=["summary"],
    summary="Получить компактную JSON-сводку по CSV-файлу",
    description="Возвращает размеры, quality_score, список проблемных колонок и флаги качества.",
)
async def get_summary_from_csv(file: UploadFile = File(...)) -> JSONResponse:
    """
    Эндпоинт, аналогичный CLI-опции --json-summary:
    принимает CSV и возвращает структурированную сводку в JSON.
    """
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
    ...
    # Анализ через ваше ядро
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    quality_flags = compute_quality_flags(summary, missing_df)

    # Собираем проблемные колонки (точно как в cli.py)
    min_missing_share = 0.1  # можно вынести в параметр, но для простоты — фиксировано
    ...
    if not missing_df.empty:
        ...
        problematic_columns.update(problematic_missing)
    ...
    # Формируем ответ — точно как в summary.json
    response_data = {
